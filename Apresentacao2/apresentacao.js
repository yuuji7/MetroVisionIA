...
(function slidePipeline() {
  const slide = pres.addSlide();
  darkBg(slide);
  addSlideTitle(slide, "Pipeline Simplificado de Visão Computacional", "Do frame capturado à informação operacional — Mapeamento em 3 fases");
  ...
  const phases = [
    {
      ...
      icon: "📹",
      title: "1. Entrada & Pré-processamento",
      steps: [
        "Captura de Frame (OpenCV): Lê a imagem da câmera em formato BGR do vídeo a 33 FPS.",
        "Conversão (BGR → RGB): Inverte a ordem das cores para compatibilidade total com a Inteligência Artificial.",
        "Resize Inteligente (LANCZOS): Reduz a imagem para 254x144 pixels com filtro anti-serrilhado, acelerando a IA sem perder contornos importantes."
      ]
    },
    {
      x: 4.75,
      ...
      icon: "🧠",
      title: "2. Núcleo de Inteligência (IA)",
      steps: [
        "Segment Anything (SAM 3.1): O modelo Vit (Vision Transformer) da Meta extrai o mapa de características visuais da imagem.",
        "Prompting de Texto CLIP: Cruza a imagem com a palavra 'person' para segmentar zero-shot cada passageiro individualmente.",
        "Aceleração FlashAttention: Otimiza o processamento matemático na GPU, reduzindo o tempo de 24.8s para apenas 0.43s por frame!"
      ]
    },
    {
      x: 8.9,
      ...
      icon: "🖥️",
      title: "3. Saída, Interface & Registro",
      steps: [
        "Alpha Blending (Composição): Mescla as máscaras coloridas semi-transparentes (40% opacidade) sobre cada passageiro identificado.",
        "Interface Gráfica (GUI): Exibe a imagem processada com a contagem total de pessoas na tela do operador.",
        "Planilha Excel Auditável: Registra a data/hora, número de passageiros e insere uma foto em miniatura do vagão automaticamente."
      ]
    }
  ];

  phases.forEach((p) => {
    // Adiciona o card de fundo para a coluna
    addCard(slide, { x: p.x, y: 1.7, w: 3.8, h: 4.9, borderColor: p.borderColor });

    // Ícone da fase
    slide.addText(p.icon, { x: p.x + 0.2, y: 1.8, w: 0.8, h: 0.5, fontSize: 32 });

    // Título da fase
    slide.addText(p.title, {
      x: p.x + 0.2, y: 2.3, w: 3.4, h: 0.4,
      fontSize: 14, color: p.borderColor, fontFace: "Segoe UI", bold: true,
    });

    // Passos formatados com espaçamento elegante
    p.steps.forEach((step, idx) => {
      ...
      slide.addText([
        { text: "▸ ", options: { bold: true, color: p.borderColor } },
        { text: title, options: { bold: true, color: COLORS.white } },
        { text: desc, options: { color: COLORS.lightGray } }
      ], {
        x: p.x + 0.2, y: 2.8 + idx * 1.35, w: 3.4, h: 1.25,
        fontSize: 10, fontFace: "Segoe UI", lineSpacingMultiple: 1.15, valign: "top"
      });
    });
  });

  // Setas horizontais para mostrar o fluxo entre colunas
  slide.addText("→", {
    x: 4.4, y: 3.6, w: 0.35, h: 0.5,
    fontSize: 24, color: COLORS.midGray, align: "center", valign: "middle"
  });
  slide.addText("→", {
    x: 8.55, y: 3.6, w: 0.35, h: 0.5,
    fontSize: 24, color: COLORS.midGray, align: "center", valign: "middle"
  });

# -*- coding: utf-8 -*-
"""
Interface gráfica Tkinter e loop de processamento em tempo real do Camera Segmenter.

TAGS DE APRESENTAÇÃO APLICADAS NA CLASSE E MÉTODOS:
- [AQUISIÇÃO_E_AMOSTRAGEM] - Cap. de frames e ciclo de 30ms/frame (~33 FPS) via OpenCV
- [CONVERSÃO_DE_ESPAÇO_DE_CORES] - Conversão de BGR (OpenCV) para RGB (PIL / SAM)
- [REDIMENSIONAMENTO_E_INTERPOLAÇÃO] - Redimensionamento inteligente da imagem
- [ANTI_ALIASING_E_SUBAMOSTRAGEM] - Uso do filtro LANCZOS anti-aliasing antes da redução
- [SEGMENTAÇÃO_SEMÂNTICA] - Chamada do pipeline de segmentação zero-shot do SAM 3.1
- [FEATURE_EXTRACTION] - ViT Backbone via set_image()
- [OTIMIZAÇÃO_DE_PIPELINE_GPU] - Uso de bfloat16 mixed precision autocast na inferência
- [GROUNDING_TEXTO_IMAGEM] - set_text_prompt() com alinhamento semântico de linguagem
- [DETECÇÃO_DE_INSTÂNCIAS] - Extração de máscaras e bounding boxes da predição
- [LATÊNCIA_E_TEMPO_REAL] - Medição e log da latência de inferência (< 1 segundo)
"""
import os
import gc
import sys
...
from .config import SAM3_AVAILABLE, SAM3_IMPORT_ERROR, build_sam3_image_model, Sam3Processor
...
class SAM3CameraSegmenterApp(tk.Tk):
    def __init__(self):
        ...
        # Inicializar banco de dados JSON e carregar configurações persistentes
        self.db_data = load_db()
        
        self.create_widgets()
        
        # Preencher campos do Metrô com dados salvos no banco de dados JSON
        self.load_metro_settings()
    ...
    def create_widgets(self):
        ...
        # Linha 5: Configurações de Lotação do Metrô
        tk.Label(ctrl_frame, text="🚇 Metrô Configs:", bg="#252526", fg="#00ff66", font=("Helvetica", 9, "bold")).grid(row=5, column=0, sticky="w", pady=5)
        
        metro_config_subframe = tk.Frame(ctrl_frame, bg="#252526")
        metro_config_subframe.grid(row=5, column=1, columnspan=5, sticky="w", pady=5)
        
        tk.Label(metro_config_subframe, text="Lugares (Assentos):", bg="#252526", fg="#ffffff", font=("Helvetica", 9)).pack(side="left", padx=(0, 2))
        self.entry_seats = ttk.Entry(metro_config_subframe, width=5)
        self.entry_seats.pack(side="left", padx=(0, 10))
        
        tk.Label(metro_config_subframe, text="Lotação (>x):", bg="#252526", fg="#ffffff", font=("Helvetica", 9)).pack(side="left", padx=(0, 2))
        self.entry_overcapacity = ttk.Entry(metro_config_subframe, width=5)
        self.entry_overcapacity.pack(side="left", padx=(0, 10))
        
        tk.Label(metro_config_subframe, text="Máximo Absoluto:", bg="#252526", fg="#ffffff", font=("Helvetica", 9)).pack(side="left", padx=(0, 2))
        self.entry_max = ttk.Entry(metro_config_subframe, width=5)
        self.entry_max.pack(side="left", padx=(0, 10))
        
        tk.Label(metro_config_subframe, text="Escala Início (%):", bg="#252526", fg="#ffffff", font=("Helvetica", 9)).pack(side="left", padx=(0, 2))
        self.entry_start_pct = ttk.Entry(metro_config_subframe, width=5)
        self.entry_start_pct.pack(side="left", padx=(0, 10))
        ...
        # Botão para abrir o painel em HTML5 do Metrô
        self.btn_open_dashboard = tk.Button(info_frame, text="🚇 Abrir Dashboard Metrô", bg="#007acc", fg="#ffffff", font=("Helvetica", 9, "bold"), activebackground="#005999", relief="flat", padx=12, command=self.open_dashboard)
    ...
    def load_metro_settings(self):
        """Popula os campos da GUI com os valores salvos no JSON database."""
        settings = self.db_data.get("settings", {})
        self.entry_seats.insert(0, str(settings.get("capacity_seats", 5)))
        self.entry_overcapacity.insert(0, str(settings.get("overcapacity_threshold", 5)))
        self.entry_max.insert(0, str(settings.get("absolute_max", 40)))
        self.entry_start_pct.insert(0, str(settings.get("occupancy_start_pct", 30)))

    def get_metro_settings(self):
        """Lê os valores inseridos na interface de forma segura com fallback."""
        try:
            capacity = int(self.entry_seats.get().strip())
            overcapacity = int(self.entry_overcapacity.get().strip())
            absolute_max = int(self.entry_max.get().strip())
            start_pct = int(self.entry_start_pct.get().strip())
            
            # Validações mínimas de consistência
            if capacity <= 0: capacity = 5
            if overcapacity < 0: overcapacity = capacity
            if absolute_max <= 0: absolute_max = 40
            if start_pct < 0 or start_pct > 100: start_pct = 30
            
            return {
                "capacity_seats": capacity,
                "overcapacity_threshold": overcapacity,
                "absolute_max": absolute_max,
                "occupancy_start_pct": start_pct
            }
        except ValueError:
            # Em caso de erro na conversão de formato, retorna os dados em cache do JSON
            return self.db_data.get("settings", {
                "capacity_seats": 5,
    ...
    def _init_camera_thread(self, cam_idx):
        # [AQUISIÇÃO_E_AMOSTRAGEM] Conexão e inicialização do dispositivo de captura física usando OpenCV
        ...
        # [AQUISIÇÃO_E_AMOSTRAGEM] Iniciar loop de atualização de quadros a cada 30ms (~33 FPS)
        self.after(30, self.update_camera_loop)

    def _init_video_thread(self, video_path):
        # [AQUISIÇÃO_E_AMOSTRAGEM] Abertura de arquivo local de vídeo para leitura temporal
    ...
    def update_camera_loop(self):
        ...
        # [AQUISIÇÃO_E_AMOSTRAGEM] Aquisição contínua do quadro (frame) da câmera
        ...
        # [AQUISIÇÃO_E_AMOSTRAGEM] Mantém atualizando com intervalo regulado de 30ms
        self.after(30, self.update_camera_loop)

    def display_opencv_frame(self, frame):
        # [CONVERSÃO_DE_ESPAÇO_DE_CORES] Converte de BGR (Padrão OpenCV) para RGB (Padrão PIL/GUI/SAM 3.1)
    ...
    def _process_snapshot_thread(self, frame, prompt, video_time):
        try:
            ...
            # [CONVERSÃO_DE_ESPAÇO_DE_CORES] Conversão de cores BGR -> RGB do frame estático capturado
            ...
            # [REDIMENSIONAMENTO_E_INTERPOLAÇÃO] & [ANTI_ALIASING_E_SUBAMOSTRAGEM] 
            # Reamostragem LANCZOS atuando como filtro passa-baixa anti-aliasing antes do downscale
            working_img = orig_img.resize((target_w, target_h), Image.Resampling.LANCZOS)
                
            user_thresh = self.val_confidence.get()
            
            # [SEGMENTAÇÃO_SEMÂNTICA] Inicialização da inferência zero-shot com SAM 3.1
            state = {}
            self.processor.set_confidence_threshold(user_thresh, state)
            
            with torch.inference_mode():
                if device == "cuda":
                    # [OTIMIZAÇÃO_DE_PIPELINE_GPU] Execução com precisão mista bfloat16 acelerada por hardware (RTX)
                    with torch.autocast("cuda", dtype=torch.bfloat16):
                        # [FEATURE_EXTRACTION] Vision Transformer (ViT) Backbone extraindo o mapa visual de alta dimensão
                        state = self.processor.set_image(working_img, state)
                        # [GROUNDING_TEXTO_IMAGEM] Alinhamento multimodal texto-imagem do prompt usando o encoder CLIP
                        state = self.processor.set_text_prompt(prompt, state)
                else:
                    state = self.processor.set_image(working_img, state)
                    state = self.processor.set_text_prompt(prompt, state)
                    
            # [LATÊNCIA_E_TEMPO_REAL] Cálculo e log de latência do processamento por frame (Inferência < 1s/frame)
            elapsed = time.time() - start_time
            
            # [DETECÇÃO_DE_INSTÂNCIAS] Extração das bounding boxes, máscaras binárias e scores gerados
            ...
            if num_detected > 0:
                ...
                # [DETECÇÃO_DE_INSTÂNCIAS] Separação de caixas e máscaras filtradas por score para a plotagem
            ...
            # Salvar print com máscara em disco para auditoria com ID ÚNICO e referenciado por nome
            unique_id = str(uuid.uuid4())[:8]  # ID compacto e altamente único de 8 caracteres
            ...
            self.log(f"[CAPTURA] Imagem salva como '{img_filename}' (ID: {unique_id})")
            
            # 6. Salvar Informações no JSON Database
            self.save_to_json(unique_id, num_detected, prompt, video_time, img_filename)
    ...
    def draw_results(self, original_img, masks, boxes, scores):
        # Delegação para a função modular de visualização
        return draw_results(original_img, masks, boxes, scores)

    def save_to_json(self, unique_id, count, prompt, video_time, img_filename):
        """Salva a nova medição no arquivo JSON e exporta o JS em tempo real."""
        # Obter os parâmetros atuais da interface do Metrô de forma thread-safe
        settings = self.get_metro_settings()
        self.db_data["settings"] = settings
        
        # Calcular porcentagem de ocupação relativa aos assentos disponíveis
        capacity = settings["capacity_seats"]
        overcapacity = settings["overcapacity_threshold"]
        start_pct = settings["occupancy_start_pct"]
        ...
        # Determinar classificação de status
        ...
        self.db_data["history"].append(new_record)
        
        # Salvar banco de dados JSON e sincronizar Javascript para leitura em tempo real no dashboard
        save_db(settings, self.db_data["history"])
        self.log(f"[JSON] Medição gravada! ID: {unique_id} | Qtd: {count} | Status: {status} ({occupancy_pct}%)")
    ...
    def open_dashboard(self):
        ...
        if os.path.exists(dashboard_path):
            try:
                ...
                self.log(f"[DASHBOARD] Abrindo painel do metrô '{dashboard_path}' no navegador...")
            except Exception as e:
                messagebox.showerror("Erro ao Abrir", f"Não foi possível abrir o dashboard:\n{e}")
        else:
            messagebox.showerror("Erro", "Arquivo dashboard.html não encontrado no diretório raiz.")
    ...
    def on_closing(self):
        ...
        # Salva as últimas configurações de limite inseridas pelo usuário antes de fechar o app
        settings = self.get_metro_settings()
        save_db(settings, self.db_data.get("history", []))
